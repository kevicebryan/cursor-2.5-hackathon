...
export default function PlaySection() {
  ...
  const hearts = useAppSelector((s) =>
    Math.min(MAX_HEARTS, Math.max(0, s.profile.profile?.hearts ?? MAX_HEARTS)),
  );
  ...
  const visibleFactCount = Math.min(
    (artifact?.fun_facts ?? []).length,
